...
import { type RuleProp, type EditCategory} from '../testData'
...
watch(currentCategory, () => {
    if (currentCategory.value === 'profile') {
        nameValue.value = userStore.value.nickName || '';
        descValue.value = userStore.value.description || '';
        ...
        nameValue.value = currentColumn.value.title;
        descValue.value = currentColumn.value.description;
...
const switchTab = (which: EditCategory) => {
    ...
    if (formRef.value) {
        formRef.value.clearAllStatus(); // 确保 `formRef` 存在后再调用
    }
...
const onFormSubmit = async (result: boolean) => {
    if (result) {
        ...
        if (currentCategory.value === 'profile') {
            ...
            await store.dispatch('fetchColumn', currentColumn.value._id)
        } else {
            payload = {
